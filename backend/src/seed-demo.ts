...
async function seedDemoData() {
  ...
  for (let i = 0; i < scotchCount; i++) {
    ...
    WhiskeyModel.create({
      ...
      tasting_notes:
        scotch.region === 'Islay' ? 'Peaty, smoky, maritime' : 'Fruity, smooth, complex',
      nose_notes:
        scotch.region === 'Islay' ? 'Peat smoke, iodine, sea salt' : 'Fruit, vanilla, honey',
  ...
  for (let i = 0; i < curatorCount; i++) {
    ...
    let country = 'USA';
    if ((scotchData as any[]).includes(whiskey)) {
      type = WhiskeyType.SCOTCH;
      country = 'Scotland';
    }
    if ((japaneseData as any[]).includes(whiskey)) {
      type = WhiskeyType.JAPANESE;
      country = 'Japan';
    }
    if ((irishData as any[]).includes(whiskey)) {
      type = WhiskeyType.IRISH;
      country = 'Ireland';
    }
    if ((ryeData as any[]).includes(whiskey)) {
