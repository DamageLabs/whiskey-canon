...
export function DashboardPage() {
	...
	async function handleBulkDelete() {
		...
		if (
			!confirm(
				`Are you sure you want to delete ${selectedIds.size} whiskey(s)?`,
			)
		) {
			return;
		}
			...
			<div className="container-fluid px-4 py-4">
							...
							{filteredWhiskeys.length !== whiskeys.length ? (
								<>
									...
									<strong style={{ color: "var(--amber-500)" }}>
										{filteredWhiskeys.length}
									</strong>{" "}
									of {pagination ? pagination.total : whiskeys.length} whiskeys
									(filtered)
								</>
							) : pagination ? (
								<>
				...
				{importResult && (
					...
					>
						...
						<p className="mb-2">
							<strong>Summary:</strong> {importResult.summary.total} rows
							processed
						</p>
						<ul className="mb-2">
							<li>
								<strong>{importResult.summary.imported}</strong> whiskeys
				...
				{!loading && filteredWhiskeys.length > 0 && (
					<div className="mb-3">
						<button
							...
							style={{
								backgroundColor: showEnhancedStats
									? "var(--amber-600)"
									: "var(--zinc-800)",
						...
						>
							<i
								className={`bi bi-${showEnhancedStats ? "bar-chart" : "graph-up"} me-2`}
							></i>
