...
router.post(
	...
	async (req: AuthRequest, res: Response) => {
		try {
			const backupDir =
				config.backupDir || path.join(__dirname, "../../backups");
	...
	(req: AuthRequest, res: Response) => {
		try {
			...
			try {
				...
				return res
					.status(400)
					.json({ error: "Backup file is not a valid SQLite database" });
			}
			...
			try {
				...
				if (tablesToRestore.length === 0) {
					...
					return res
						.status(400)
						.json({ error: "Backup contains no matching tables" });
				}
	...
	(req: AuthRequest, res: Response) => {
		try {
			...
			if (!magic.startsWith("SQLite format 3")) {
				return res
					.status(400)
					.json({ error: "File is not a valid SQLite database" });
			}
