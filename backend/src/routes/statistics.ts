  ...
  try {
    ...
    const financialStats = db
      ...
      SELECT
        ...
        COALESCE(SUM(purchase_price * COALESCE(quantity, 1)), 0) as total_spent,
        COALESCE(SUM(msrp * COALESCE(quantity, 1)), 0) as total_msrp,
        COALESCE(SUM(current_market_value * COALESCE(quantity, 1)), 0) as total_current_value,
        COALESCE(SUM(secondary_price * COALESCE(quantity, 1)), 0) as total_secondary_value,
