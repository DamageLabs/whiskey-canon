...
const bourbonReplacements = [
  ...
  {
    name: 'Wilderness Trail Small Batch',
  ...
  {
    name: 'Barrell Bourbon Batch 033',
  ...
  {
    name: 'Blue Note Juke Joint',
    distillery: 'Blue Note Bourbon',
    age: 15,
    abv: 50.0,
    msrp: 149.99,
    secondary: 175,
  ...
  {
    name: 'Old Carter Whiskey Co Batch 10',
...
const irishReplacements = [
  ...
  {
    name: 'Tyrconnell 10 Year Madeira Finish',
...
async function removeDuplicates() {
    ...
    for (const dup of duplicates) {
      ...
      for (const replaceId of replaceIds) {
        ...
        if (user.username === 'bourbon_lover') {
          availableReplacements = bourbonReplacements.filter(
            (r) => !existingSet.has(`${r.name}|${r.distillery}`)
          );
          if (availableReplacements.length === 0) {
            // Fallback to rye if all bourbons are taken
            availableReplacements = ryeReplacements.filter(
              (r) => !existingSet.has(`${r.name}|${r.distillery}`)
            );
            if (availableReplacements.length === 0) {
          ...
          availableReplacements = scotchReplacements.filter(
            (r) => !existingSet.has(`${r.name}|${r.distillery}`)
          );
          if (availableReplacements.length === 0) {
            // Fallback to Irish if all scotches are taken
            availableReplacements = irishReplacements.filter(
              (r) => !existingSet.has(`${r.name}|${r.distillery}`)
            );
            if (availableReplacements.length === 0) {
              console.log(
                `  ⚠️  Warning: No unique scotch/Irish replacements for ${user.username}`
              );
              availableReplacements = scotchReplacements;
            }
          }
          replacement = randomPick(availableReplacements);
          newType = irishReplacements.includes(replacement)
            ? WhiskeyType.IRISH
            : WhiskeyType.SCOTCH;
        ...
        const country =
          newType === WhiskeyType.SCOTCH
            ? 'Scotland'
            : newType === WhiskeyType.IRISH
              ? 'Ireland'
              : newType === WhiskeyType.JAPANESE
                ? 'Japan'
                : 'USA';
        ...
        `
          ...
          newType === WhiskeyType.SCOTCH && (replacement as any).region === 'Islay'
            ? 'Peaty and smoky'
            : 'Smooth and complex',
          country,
          size,
  ...
  for (const user of users) {
    ...
    if (remaining.length > 0) {
      ...
      const total = db
        .prepare('SELECT COUNT(*) as count FROM whiskeys WHERE created_by = ?')
        .get(user.id) as any;
