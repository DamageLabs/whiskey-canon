...
router.get(
	...
	(req: AuthRequest, res: Response) => {
		...
		try {
			...
			const { data, total } = WhiskeyModel.search(
	...
	(req: AuthRequest, res: Response) => {
		try {
			...
			const mapSubcategoryToType = (subcategory: string): string => {
				...
				if (
					sub.includes("single malt") ||
					sub.includes("blended") ||
					sub.includes("single grain")
				)
					return "scotch";
			...
			for (let i = 1; i < lines.length; i++) {
				try {
					...
					headers.forEach((header, index) => {
						...
						switch (header) {
							...
							case "Status":
								...
								break;
							case "Is Opened":
								whiskeyData.is_opened =
									value.toLowerCase() === "yes" || value === "1";
								break;
							case "Date Opened":
							...
							case "Is Investment Bottle":
								whiskeyData.is_investment_bottle =
									value.toLowerCase() === "yes" || value === "1";
								break;
							case "Mash Bill":
							...
							case "Limited Edition":
								whiskeyData.limited_edition =
									value.toLowerCase() === "yes" || value === "1";
								break;
							case "Chill Filtered":
								whiskeyData.chill_filtered =
									value.toLowerCase() === "yes" || value === "1";
								break;
							case "Natural Color":
								whiskeyData.natural_color =
									value.toLowerCase() === "yes" || value === "1";
								break;
							case "Is For Sale":
								whiskeyData.is_for_sale =
									value.toLowerCase() === "yes" || value === "1";
								break;
							case "Asking Price":
					...
					if (
						!whiskeyData.name ||
						!whiskeyData.type ||
						!whiskeyData.distillery
					) {
						skipped.push(
							`Row ${i + 1}: Missing required fields (name, type, or distillery)`,
						);
						continue;
					}
					...
					if (
						!Object.values(WhiskeyType).includes(
							whiskeyData.type as WhiskeyType,
						)
					) {
						skipped.push(
							`Row ${i + 1}: Invalid whiskey type "${whiskeyData.type}"`,
						);
						continue;
					}
...
router.get(
	...
	param("id")
		.isInt({ min: 1 })
		.withMessage("Whiskey ID must be a positive integer"),
	validate,
	(req: AuthRequest, res: Response) => {
		try {
			const whiskey = WhiskeyModel.findById(
...
router.post(
	...
	[
		...
		body("type")
			.isIn(Object.values(WhiskeyType))
			.withMessage("Invalid whiskey type"),
		...
		body("msrp")
			.optional()
			.isFloat({ min: 0 })
			.withMessage("MSRP must be a positive number"),
...
router.delete(
	...
	(req: AuthRequest, res: Response) => {
		try {
			const deleted = WhiskeyModel.delete(
				parseInt(req.params.id as string),
				req.user!.id,
			);
